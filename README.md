...
TTY 中会以彩色有界区域持续重画完整 DAG。宽终端按拓扑层级从左到右画出分叉、汇合、跨层依赖和独立分支；空间不足时先缩短节点标签，再退化成仍保持连通关系的纵向轨道图，而不是在末尾罗列 from/to 边。节点下方保留工具、目标、当前状态、审批状态、耗时、冲突、上游失败和实际发生的 Bash/文件并发警告。审批提示出现前图会暂停，后续状态在提示下方恢复，避免覆盖输入。管道或 CI 环境输出稳定的 `[code-dag]` 行；`NO_COLOR=1` 可关闭颜色。

模型返回的 `reasoning_content` 默认以灰色 `[thinking]` 流实时显示；`--verbose` 额外显示 Turn 状态，不再决定 thinking 是否可见。
